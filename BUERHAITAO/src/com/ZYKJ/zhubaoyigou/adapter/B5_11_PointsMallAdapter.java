...
public class B5_11_PointsMallAdapter extends BaseAdapter {
	...
	public View getView(int position, View cellView, ViewGroup arg2) {
		...
		if (cellView == null) {
			cellView = listContainer.inflate(
					R.layout.ui_points_mall_list_items, null);
		}
		TextView tv_productName = (TextView) cellView
				.findViewById(R.id.tv_productName);
		TextView tv_productIntro = (TextView) cellView
				.findViewById(R.id.tv_productIntro);
		TextView tv_productPoints = (TextView) cellView
				.findViewById(R.id.tv_productPoints);
		...
		// }
		ImageView iv_product = (ImageView) cellView
	...
	class ExchangeListener implements View.OnClickListener {
		...
		public void onClick(View v) {
			// TODO Auto-generated method stub
			int a = Integer.parseInt(allpoint);
			int b = Integer.parseInt(data.get(position).get("pgoods_points"));
			if (a < b) {
				Toast.makeText(c, "很抱歉，您的积分不够兑换此商品", Toast.LENGTH_LONG).show();
			} else {
				...
				Button btn_yesButton = (Button) window
						.findViewById(R.id.btn_yes);
				Button btn_noButton = (Button) window.findViewById(R.id.btn_no);
				TextView dialog_notic_text = (TextView) window
						.findViewById(R.id.dialog_notic_text);
				dialog_notic_text.setText("确定使用"
						+ data.get(position).get("pgoods_points") + "积分兑换"
						+ data.get(position).get("pgoods_name"));
				btn_yesButton.setOnClickListener(new OnClickListener() {

					@Override
					public void onClick(View arg0) {
						// TODO Auto-generated method stub
						// Toast.makeText(c, "跳转到兑换详情",
						// Toast.LENGTH_LONG).show();
						Intent intent_to_exchangepoints = new Intent(c,
								B5_11_1_ExchangeDetail.class);

						Bundle bundle = new Bundle();
						bundle.putString("pgoods_image", data.get(position)
								.get("pgoods_image"));
						bundle.putString("pgoods_name",
								data.get(position).get("pgoods_name"));
						bundle.putString("pgoods_body",
								data.get(position).get("pgoods_body"));
						bundle.putString("pgoods_points", data.get(position)
								.get("pgoods_points"));
						bundle.putString("pgoods_id",
								data.get(position).get("pgoods_id"));
